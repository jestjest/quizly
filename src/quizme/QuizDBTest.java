...
public class QuizDBTest {
	...
	public static void oneTimeSetUp() {
		...
		quizDB.removeQuiz(3); /* keep replacing quiz 3 in basic test */
		quizDB.removeQuiz(4); /* keep replacing quiz 4 in basic test */
	}
	...
	public void basictest() {
		...
		Timestamp anotherDate = new Timestamp(2016 - 1900, 02, 27, 5, 0, 0, 0);
		...
		Timestamp yetAnotherDate = new Timestamp(2016 - 1900, 02, 28, 0, 0, 0, 0);
		int quizid3 = quizDB.addQuiz("Fake3", "another example for testing", 5, "LB", yetAnotherDate, true, true, true, true, 45);
		assertEquals(quizid3, 3);
		
		int quizid4 = quizDB.addQuiz("Fake4", "another example for testing", 5, "JB", yetAnotherDate, true, true, true, true, 45);
		assertEquals(quizid4, 4);
		...
		rs = quizDB.getRecentQuizzesModifiedHelper("LB", 3, anotherDate);
		try {
			int count = 0;
			while(rs.next()) {
				count++;
				assertTrue(rs.getString("name").equals("Fake3"));
			}
		...
		rs = quizDB.getRecentQuizzesModifiedHelper(3, anotherDate);
		try {
			int count = 0;
			while(rs.next()) {
				count++;
